# Version: 1.1
# Description: Main experiment. Includes normal SafeOpt and Gradient-based alignment
...
# Create directories for Z space
if not os.path.exists('agent_data_3A_Z'): 
    os.makedirs('agent_data_3A_Z')
if not os.path.exists('data_3A_Z'): 
    os.makedirs('data_3A_Z')
...
# Quarc Experiment
def run_experiment(kp1, kd1, kp2, kd2, kp3, kd3, iteration, is_z_space=False):
    ...
    retrieve_data(target_uri_1, modelName, gain_arg1, std_args, 1, iteration, is_z_space)
    retrieve_data(target_uri_2, modelName, gain_arg2, std_args, 2, iteration, is_z_space)
    retrieve_data(target_uri_3, modelName, gain_arg3, std_args, 3, iteration, is_z_space)

    data_dir = 'data_3A_Z' if is_z_space else 'data_3A'
    rt_t1, rt_theta1, theta_d = load_agent_data(f'{data_dir}/servoPDF-1_{iteration}.mat')
    rt_t2, rt_theta2, _ = load_agent_data(f'{data_dir}/servoPDF-2_{iteration}.mat')
    rt_t3, rt_theta3, _ = load_agent_data(f'{data_dir}/servoPDF-3_{iteration}.mat')
...
for iteration in range(1, N+1):
    ...
    y, os1, os2, os3 = run_experiment(K1_next[0], Kd1, K2_next[0], Kd2, K3_next[0], Kd3, iteration, is_z_space=False)
...
# Add before Z-space optimization loop
with open(f'{agent_data_dir}_Z/agent1_data.txt', 'w', newline='') as f1, \
     open(f'{agent_data_dir}_Z/agent2_data.txt', 'w', newline='') as f2, \
     open(f'{agent_data_dir}_Z/agent3_data.txt', 'w', newline='') as f3:
    writer1 = csv.writer(f1)
    writer2 = csv.writer(f2)
    writer3 = csv.writer(f3)
    writer1.writerow(['Iteration', 'Z', 'Kp', 'Reward'])
    writer2.writerow(['Iteration', 'Z', 'Kp', 'Reward'])
    writer3.writerow(['Iteration', 'Z', 'Kp', 'Reward'])
...
for iteration in range(0, N2):
    ...
    y, os1, os2, os3 = run_experiment(Kp1_next[0], Kd1, Kp2_next[0], Kd2, Kp3_next[0], Kd3, iteration, is_z_space=True)
    ...
    with open(f'{agent_data_dir}_Z/agent1_data.txt', 'a', newline='') as f1, \
        open(f'{agent_data_dir}_Z/agent2_data.txt', 'a', newline='') as f2, \
        open(f'{agent_data_dir}_Z/agent3_data.txt', 'a', newline='') as f3:
        writer1 = csv.writer(f1)
        writer2 = csv.writer(f2)
        writer3 = csv.writer(f3)
        writer1.writerow([iteration, Z1_next[0], Kp1_next[0], y])
        writer2.writerow([iteration, Z2_next[0], Kp2_next[0], y])
        writer3.writerow([iteration, Z3_next[0], Kp3_next[0], y])

    with open(f'{agent_data_dir}_Z/rewards.txt', 'a') as f:
        f.write(f"{iteration},{y}\n")
