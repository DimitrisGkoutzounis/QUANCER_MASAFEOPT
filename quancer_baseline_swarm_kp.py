...
def retrieve_data(target_uri, modelName, gain_arg, std_args, agent, iteration):
    ...
    # Create 'data_baseline_kp' directory if it doesn't exist
    data_dir = 'data_3A_baseline_kp'  
...
if not os.path.exists('plots_3A_baseline_kp'):
    os.makedirs('plots_3A_baseline_kp')
if not os.path.exists('data_3A_baseline_kp'): 
    os.makedirs('data_3A_baseline_kp')
if not os.path.exists('agent_data_3A_baseline_kp'): 
    os.makedirs('agent_data_3A_baseline_kp')
...
rt_t1, rt_theta1, theta_d = load_agent_data('data_3A_baseline_kp/servoPDF-1_0.mat') 
rt_t2, rt_theta2, _ = load_agent_data('data_3A_baseline_kp/servoPDF-2_0.mat')       
rt_t3, rt_theta3, _ = load_agent_data('data_3A_baseline_kp/servoPDF-3_0.mat')       
...
class Agent:
    def __init__(self, id, bounds, safe_point, initial_reward):
        ...
        self.x0 = np.asarray([safe_point]).reshape(-1,3)
...
# Kp bounds
K_bounds = [(0.01, 10),(0.01,10), (0.01, 10)]
...
# Quarc Experiment
def run_experiment(kp1, kp2, kp3, iteration):
    
    # Set gain arguments
    gain_arg1 = f' -Kp {kp1} -Kd {kd1_0}'
    gain_arg2 = f' -Kp {kp2} -Kd {kd2_0}'
    gain_arg3 = f' -Kp {kp3} -Kd {kd3_0}'
    ...
    rt_t1, rt_theta1, theta_d = load_agent_data(f'data_3A_baseline_kp/servoPDF-1_{iteration}.mat') 
    rt_t2, rt_theta2, _ = load_agent_data(f'data_3A_baseline_kp/servoPDF-2_{iteration}.mat')       
    rt_t3, rt_theta3, _ = load_agent_data(f'data_3A_baseline_kp/servoPDF-3_{iteration}.mat')     
...
agent_data_dir = 'agent_data_3A_baseline_kp'  
...
with open(f'{agent_data_dir}/agent1_data.txt', 'w', newline='') as f1, \
    ...
    writer1.writerow(['Iteration', 'Kp','Reward'])
    writer2.writerow(['Iteration', 'Kp','Reward'])
    writer3.writerow(['Iteration', 'Kp','Reward'])  
    writer1.writerow([0, x0, reward_0])
    writer2.writerow([0, x0, reward_0])
    writer3.writerow([0, x0, reward_0]) 
...
for iteration in range(1, N+1):
    ...
    print(f"Iteration {iteration}, Agent 1:  -Kp {K1_next[0]}, Agent 2: -Kp {K1_next[1]}, Agent 3: -Kp {K1_next[2]}")

    # Run the experiment with kp1_next, kp2_next, kp3_next
    y, os1, os2, os3 = run_experiment(K1_next[0], K1_next[1], K1_next[2], iteration)
    ...
    with open(f'{agent_data_dir}/agent1_data.txt', 'a', newline='') as f1, \
        ...
        writer2.writerow([iteration, K1_next[1], y])
        writer3.writerow([iteration, K1_next[2], y])  
...
# plt.grid(True)
# plt.savefig('plots_3A_baseline_kp/reward_over_iterations.png')  
# plt.show()
